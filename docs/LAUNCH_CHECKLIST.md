...
- Creator profile system in the engine (`profiles/`, `--profile`,
  ...
  packages. Comment-safe yaml parsing (quoted `#RRGGBB` colours now work,
...
- CI (`.github/workflows/release.yml`): tag `ryan-v0.1.0` or `pse-v0.1.0`
  (or run manually) -> Mac dmg + Windows installer with the frozen engine,
...
3. Download installers from the GitHub release, install on your own Mac,
   run one real edit per profile.
...
## To sharpen the Ryan profiles (approved; this is an accuracy upgrade)
...
4. Re-run a couple of held-out raws and eyeball side by side.
